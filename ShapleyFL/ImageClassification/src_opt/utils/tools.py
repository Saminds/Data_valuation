...
def get_noiseword():
    NoiseWord = ["0_NonIID", "1_LongTail", "2_LabelNoise", "3_LabelNoise2", "4_DataNoise", "5_GradientNoise", "6_RandomAttack", "7_ReverseGradient", "8_ConstantAttack", "9_ScalingNoise"]
    return NoiseWord
...
def add_gradient_noise_new(args, w_dict, idxs):
    # Ensure idxs is iterable
    if not isinstance(idxs, (list, tuple, np.ndarray)):
        idxs = [idxs]  # Convert a single integer to a list

    if args.noise < 5:
        return w_dict

    # for idx in idxs:  # Now idxs is guaranteed to be iterable
    #     if idx % 4 == 0:
            # Your existing logic here
    for key in w_dict.keys():
        if args.noise == 5:
                for param_key in w_dict[key].keys():
                    # Get the shape of the current parameter tensor
                    param_shape = w_dict[key][param_key].shape
                    # Generate noise with the same shape as the current parameter tensor
                    noise = torch.tensor(np.random.normal(0, args.noiselevel, param_shape)).to(args.device)
                    # Generate a tensor of ones with the same shape as the current parameter tensor
                    ratio = torch.ones(param_shape).to(args.device)
                    # Add noise to the current parameter tensor
                    w_dict[key][param_key] = w_dict[key][param_key] * (ratio + noise)
            # noise = torch.tensor(np.random.normal(0, args.noiselevel, w_dict[key].shape)).to(args.device)
            # ratio = torch.ones(w_dict[key].shape).to(args.device)
            # w_dict[key] = w_dict[key] * (ratio + noise)
        elif args.noise == 6:
                for param_key in w_dict[key].keys():
                    # Generate noise for each parameter tensor based on its shape
                    param_shape = w_dict[key][param_key].shape
                    noise = torch.tensor(np.random.normal(0, args.noiselevel, param_shape))
                    noise = noise.to(torch.float32).to(args.device)
                    # Replace the original parameter tensor with generated noise
                    w_dict[key][param_key] = noise
            # noise = torch.tensor(np.random.normal(0, args.noiselevel, w_dict[key].shape))
            # noise = noise.to(torch.float32).to(args.device)
            # w_dict[key] = noise
        elif args.noise == 7:
                for param_key in w_dict[key].keys():
                # Scale and flip sign of each tensor within the OrderedDict
                    w_dict[key][param_key] = w_dict[key][param_key] * -10
            # w_dict[key] = w_dict[key] * -10
        elif args.noise == 8:
                for param_key in w_dict[key].keys():
        # Negate each tensor within the OrderedDict
                    w_dict[key][param_key] = -1.0 * w_dict[key][param_key]
            # w_dict[key] = torch.ones(w_dict[key].shape).to(args.device) * -1
        elif args.noise == 9:
            boosting_factor = 5 / 0.1
            for param_key in w_dict[key].keys():
                w_dict[key][param_key] = w_dict[key][param_key] * boosting_factor
            # w_dict[key] = w_dict[key] * boosting_factor
...
def avgSV_weights(w, shapley,ori):
    ...
    for key in w_avg.keys():
        for i in range(0, len(w)-1): #edited -1 instead of w
...
def unbiased_selection(p):
    idxs = []
    while(len(idxs) < 2): #ensures that the loop continues until idxs contains at least two indices
        idxs = []
        for i in range(len(p)): 
            rand = random.random()
            if rand < p[i]:
