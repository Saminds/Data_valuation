...
from src_opt.utils.tools import get_dataset, average_weights, exp_details,avgSV_weights,softmax,unbiased_selection, add_gradient_noise, add_random_gradient, get_noiseword
...
def solver(gamma):
    ...
    for epoch in range(args.epochs):#< ================ here is your shit 
        ...
        for i in range(len(shapley)):
            global_shapley[idxs_users[i]] = (1 - gamma)*global_shapley[idxs_users[i]]+gamma*(shapley[i]-min_shapley)/(max_shapley-min_shapley)

        print(idxs_users)
        print(global_shapley[idxs_users])
        rem = 0
        first_flag = True
    ...
    file_name = f'{directory}/{args.dataset}_{args.model}_{args.epochs}_C[{args.frac}]_iid[{args.iid}]_E[{args.local_ep}].pkl'
...
if __name__ == '__main__':
    ...
    for rp in range(repeat):
        ...
        f = open(path, "a+")
        f.writelines("Repetition [%d] : [%s]\n" % (rp + 1, ', '.join(["%.4f" % w for w in acc_list])))
        f.flush()
        f.close()
